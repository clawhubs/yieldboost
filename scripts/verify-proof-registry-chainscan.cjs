...
async function main() {
  ...
  const verificationPath = path.join(artifactDir, "ProofRegistry.verification.json");
  const sourcePath = path.join(rootDir, "contracts", "ProofRegistry.sol");
  ...
  if (!fs.existsSync(verificationPath)) {
    throw new Error(
      `Missing verification bundle. Run "npm run export:proof-registry-abi" first.`,
    );
  }
  ...
  console.log(`Verifying ProofRegistry on ${network} ChainScan...`);
