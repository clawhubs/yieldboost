...
function main() {
  ...
  const contractName = "YieldStrategyINFT";
  ...
  const testnetDeployment = readJsonIfExists(
    path.join(artifactDir, "yield-strategy-inft-deployment.json"),
  );
  const mainnetDeployment = readJsonIfExists(
    path.join(artifactDir, "yield-strategy-inft-deployment-mainnet.json"),
  );
