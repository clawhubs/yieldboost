...
LOCAL_SENTINEL_WALLET_KEY_FILE="${LOCAL_SENTINEL_WALLET_KEY_FILE:-/home/cucu/Coder/Private key wallet/private}"
...
append_or_replace_env "YB_SENTINEL_WALLET_KEY_FILE" "${VPS_APP_HOME}/.yieldboost-sentinel-wallet-keys"
...
if [[ -f "$LOCAL_SENTINEL_WALLET_KEY_FILE" ]]; then
  scp -q "$LOCAL_SENTINEL_WALLET_KEY_FILE" "${VPS_HOST_ALIAS}:/tmp/yieldboost-sentinel-wallet-keys"
  ssh "$VPS_HOST_ALIAS" "sudo install -m 0600 -o '${VPS_APP_USER}' -g '${VPS_APP_USER}' /tmp/yieldboost-sentinel-wallet-keys '${VPS_APP_HOME}/.yieldboost-sentinel-wallet-keys' && rm -f /tmp/yieldboost-sentinel-wallet-keys"
fi
