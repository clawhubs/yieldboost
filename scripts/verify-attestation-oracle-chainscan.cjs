...
async function main() {
  ...
  const verificationPath = path.join(artifactDir, "AttestationRegistryOracle.verification.json");
  const sourcePath = path.join(rootDir, "contracts", "AttestationRegistryOracle.sol");
  ...
  if (!fs.existsSync(verificationPath)) {
    throw new Error(
      `Missing verification bundle. Run \"node scripts/export-attestation-oracle-abi.cjs\" first.`,
    );
  }
  ...
  const payload = {
    ...
    name: "AttestationRegistryOracle",
    sourceCode,
    compiler: "0.8.24",
    license: "3",
  ...
  console.log(`Verifying AttestationRegistryOracle on ${network} ChainScan...`);
