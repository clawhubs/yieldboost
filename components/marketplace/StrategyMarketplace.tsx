...
export default function StrategyMarketplace() {
  ...
  const stats = useMemo(() => {
    const activeStrategies = strategies.filter((strategy) =>
      listingMap.has(strategy.tokenId),
    );
    const verified = activeStrategies.filter((strategy) => strategy.verified).length;
    const avgAccuracy =
      activeStrategies.length > 0
        ? activeStrategies.reduce((sum, strategy) => sum + (strategy.confidence ?? 0), 0) /
          activeStrategies.length
        : 0;
    const topRoi = activeStrategies.reduce(
      (best, strategy) => Math.max(best, strategy.yieldIncreasePct ?? 0),
      0,
    );
    const topApy = activeStrategies.reduce(
      (best, strategy) => Math.max(best, strategy.apy ?? 0),
      0,
    );
    const floorPrice = listings.reduce((lowest, listing) => {
      const price = Number(listing.price0G);
  ...
  }, [listingMap, listings, strategies]);

  const sortedStrategies = useMemo(
    () =>
      strategies
        .filter((strategy) => listingMap.has(strategy.tokenId))
        .sort(
        (left, right) =>
          new Date(right.timestamp).getTime() - new Date(left.timestamp).getTime(),
      ),
    [listingMap, strategies],
  );
    ...
    <div className="space-y-5">
        ...
        <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr] xl:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-[15px] leading-7 text-[#a8b7c3]">
              Marketplace hanya menampilkan Strategy Agent NFT yang sudah di-list on-chain. NFT yang baru dimint tetap berada di menu Agents sampai owner memasukkannya ke marketplace.
            </p>
            {message ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {[
              { label: "Listed Strategies", value: stats.total.toString(), tone: "text-white" },
              { label: "Active Listings", value: stats.listed.toString(), tone: "text-[#68ff7a]" },
              { label: "Floor Price", value: `${stats.floorPrice.toFixed(2)} 0G`, tone: "text-white" },
      ...
      {sortedStrategies.length === 0 ? (
        <section className="rounded-[28px] border border-[#17232d] bg-[#070d12] p-8 text-center">
          ...
          <p className="mt-2 text-[13px] text-[#9fb0be]">
            Minted Agent NFTs stay in the Agents page first. Open Agents, choose a wallet-owned NFT, then list it to publish it here.
          </p>
          <Link
            href="/agents"
      ...
      <section className="rounded-[24px] border border-[#15232d] bg-[#070d12] p-5">
        ...
        <p className="mt-2 text-[13px] leading-6 text-[#9fb0be]">
          This marketplace renders active on-chain listings only. Minted but unlisted Agent NFTs remain in `/agents`, where the owner can decide when to list them.
        </p>
      </section>
    </div>
  );
